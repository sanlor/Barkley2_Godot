...
func preload_skip_tutorial_save_data(): # user skip the CC and tutorial
	...
	B2_Gun.append_gun_to_bandolier( B2_Gun.get_gun_from_db("wilmerGun", "WILM") )	#scr_gun_db("wilmerGun");
	B2_Gun.append_gun_to_bandolier( B2_Gun.get_gun_from_db("estherGun", "ESTR") )	#scr_gun_db("estherGun");
	
	B2_Note.take_note( "Wilmer's Amortization Schedule" )
	B2_Database.money_change( B2_Database.money.get("wilmerMortgageTotal", 69420) )
	B2_Candy.add_candy_recipe("Butterscotch") 										#Candy("recipe add", "Butterscotch");
	B2_Candy.gain_candy("Butterscotch") 											#repeat (10) scr_items_add(scr_items_db_getCopyOfItem("Butterscotch")); 
