...
var question_id := 0
var amount_of_question := 5 # Nº of questions to be asked
...
# AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
	...
	grid_piece.position = Vector2( 208, 116 )
...
func awnser_question( choice : int):
	...
	target_x = grid_piece.position.x + answer_x[question_id][choice] * 32
	target_y = grid_piece.position.y + answer_y[question_id][choice] * 32
...
func _process(delta):
	if piece_can_move:
		...
		if round(grid_piece.position.x) == round(target_x):
			if round(grid_piece.position.y) == round(target_y):
