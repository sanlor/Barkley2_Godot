...
func _ready() -> void:
	...
	if play_cinema_at_room_start and is_instance_valid( cutscene_script ):
		...
		if B2_Playerdata.Quest("indianRopeTrick") == 1: return
		if B2_Playerdata.Quest("pelekryteState") < 2 or B2_Playerdata.Quest("pelekryteState") == 3 or B2_Playerdata.Quest("pelekryteState") >= 5: return
