...
const O_ENEMY_ATTACK_OOZE_SPLASH = preload("uid://b38k1jre2818k")

var gravity 		:= 0.025
var inaccuracy		:= 8.0

@onready var ooze: AnimatedSprite2D = $ooze
...
func _ready() -> void:
	...
	ooze.rotation = ( position.direction_to(my_target) ).angle()
	
func _physics_process(_delta: float) -> void:
	position = position.lerp( my_target, 0.075 )
	ooze.rotation = Vector2.from_angle(ooze.rotation).move_toward( Vector2.DOWN, gravity).angle()
	...
	if coll:
		for body_dict in coll:
			...
			if body is B2_CombatActor and not body == my_shooter:
				...
				print( "%s: hit unknown body %s." % [name, body] )
		_hit()
...
func _hit() -> void:
	var puddle : Area2D = O_ENEMY_ATTACK_OOZE_SPLASH.instantiate()
	puddle.global_position = global_position
	add_sibling( puddle, true )
	queue_free()
