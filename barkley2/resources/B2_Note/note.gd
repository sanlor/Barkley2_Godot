...
const ARTS 		= preload("res://barkley2/resources/B2_Note/arts.json")
const NOTES 	= preload("res://barkley2/resources/B2_Note/notes.json")
const PERSONS 	= preload("res://barkley2/resources/B2_Note/persons.json")
...
# NOTE 26/07/25 Working on it.
# Changes made to the font fucked up the note layout. Also need to add the "select" command. Thanks Joad.
...
func _ready() -> void:
	...
	#note.store_string( JSON.stringify(notes, "\t") )
	#var pers = FileAccess.open( "res://barkley2/resources/B2_Note/persons.json", FileAccess.WRITE )
...
static func clear_notes() -> void:
	B2_Config.set_user_save_data("quest.notes", Array() )
	
static func take_note( note_title : String ): # as in, hoopz receives the note
	var my_notes = B2_Config.get_user_save_data("quest.notes")
...
static func get_note_from_db( note_title : String ) -> Dictionary:
