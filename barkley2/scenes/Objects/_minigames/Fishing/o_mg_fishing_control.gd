...
func _physics_process(_delta: float) -> void:
	match curr_MODE:
		MODE.SETUP:
			_update_power_angle_data()
			if minigame_camera and minigame_lure_reticle:
				minigame_camera.position = lerp( minigame_camera.global_position, minigame_lure_reticle.global_position, 0.5 )
				
				## Avoid seeing outside the map.
				# Taken from B2_Camera
				minigame_camera.offset.x = clamp( offset.x, limit_width.x + (384.0/2.0 - minigame_camera.position.x), limit_width.y - (384.0/2.0 + minigame_camera.position.x) )
				minigame_camera.offset.y = clamp( offset.y, limit_height.x + (240.0/2.0 - minigame_camera.position.y), limit_height.y - (240.0/2.0 + minigame_camera.position.y) )
			else:
