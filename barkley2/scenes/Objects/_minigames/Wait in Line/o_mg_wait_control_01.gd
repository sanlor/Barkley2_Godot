...
@onready var o_mg_wait_spot_10: 	Sprite2D = $"../o_mg_wait_spot10"
@onready var o_mg_wait_spot_09: 	Sprite2D = $"../o_mg_wait_spot09"
@onready var o_mg_wait_spot_08: 	Sprite2D = $"../o_mg_wait_spot08"
@onready var o_mg_wait_spot_07: 	Sprite2D = $"../o_mg_wait_spot07"
@onready var o_mg_wait_spot_06: 	Sprite2D = $"../o_mg_wait_spot06"
@onready var o_mg_wait_spot_05: 	Sprite2D = $"../o_mg_wait_spot05"
@onready var o_mg_wait_spot_04: 	Sprite2D = $"../o_mg_wait_spot04"
@onready var o_mg_wait_spot_03: 	Sprite2D = $"../o_mg_wait_spot03"
@onready var o_mg_wait_spot_02: 	Sprite2D = $"../o_mg_wait_spot02"
@onready var o_mg_wait_spot_01: 	Sprite2D = $"../o_mg_wait_spot01"

@onready var o_mg_wait_spot : Array[B2_MiniGame_Mortage_Spot] = [
				o_mg_wait_spot_01,
				o_mg_wait_spot_02,
				o_mg_wait_spot_03,
				o_mg_wait_spot_04,
...
## Where a ped should face when it reach a new position.
@onready var o_mg_wait_face_direction : Dictionary[B2_MiniGame_Mortage_Spot,Vector2] = {
				o_mg_wait_spot_01: Vector2.UP,
				o_mg_wait_spot_02: Vector2.UP,
				o_mg_wait_spot_03: Vector2.UP,
				o_mg_wait_spot_04: Vector2.UP,
				o_mg_wait_spot_05: Vector2.RIGHT,
				o_mg_wait_spot_06: Vector2.DOWN,
				o_mg_wait_spot_07: Vector2.DOWN,
				o_mg_wait_spot_08: Vector2.DOWN,
				o_mg_wait_spot_09: Vector2.RIGHT,
				o_mg_wait_spot_10: Vector2.RIGHT,
				o_mg_wait_spot_11: Vector2.RIGHT,
				o_mg_wait_spot_12: Vector2.RIGHT,
...
func _check_hoopz_spot() -> int:
	var id := 0 ## in no spot
	for s in o_mg_wait_spot:
		if is_instance_valid(s.spot_holder): # Solves "Left operand of 'is' is a previously freed instance." issue.
			if s.spot_holder is B2_Player_FreeRoam:
				id = get_spot_id( s )
				break
	return id

## Used by the spots. allows the monitor to check if Hoopz is trying to cut in line.
func check_if_spot_can_register_hoopz( spot_id ) -> bool:
	## last spot is always free.
	if spot_id == 0:
		hoopz_last_known_spot = spot_id
		return true
	
	## spot on ther front is free
	if spot_id - 1 == hoopz_last_known_spot or spot_id == hoopz_last_known_spot:
		hoopz_last_known_spot = spot_id
		return true
...
func get_ped_face_direction( spot : B2_MiniGame_Mortage_Spot ) -> Vector2:
	assert( o_mg_wait_face_direction.has(spot), "Unknown spot node %s." % spot )
	return o_mg_wait_face_direction[ spot ]
